...
const ADMIN_ITEMS = [
  { to: "/admin", label: "Home", Icon: Home, tid: "nav-home" },
  { to: "/admin/shopping", label: "List", Icon: ShoppingBasket, tid: "nav-shopping" },
  { to: "/admin/settings", label: "Setup", Icon: Settings, tid: "nav-settings" },
];

export default function BottomNav() {
  const { pathname } = useLocation();

  // Navigation bar only appears when in Admin mode
  ...
  return (
    <div className="fixed left-1/2 -translate-x-1/2 bottom-4 z-50 pointer-events-none" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
      <div className="glass rounded-full px-3 py-2 flex items-center gap-1 pointer-events-auto shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
        {ADMIN_ITEMS.map(({ to, label, Icon, tid }) => (
          <NavLink
            key={to}
            to={to}
            end={to === "/admin"}
